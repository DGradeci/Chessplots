...
function []= run_chessplot(varargin) 
...
%gfp divisons 
    for i=1:length(varargin)/3
        ...
        if struct==1
            T=get_divisions(varargin{i+(length(varargin)/3)});
        else
            T=get_divisions_new(varargin{i+(length(varargin)/3)});
        end
...
% rfp divisions
    for i=1:length(varargin)/3
        ...
        %get dividing cell ID's and frames from tracked data
        if struct==1
            T=get_divisions(varargin{i+(2*(length(varargin)/3))});
        else
            T=get_divisions_new(varargin{i+(2*(length(varargin)/3))});
        end
        % select the correct N file corresponding the the position chosen
        N=varargin{i};
...
%GFP Apoptosis 
    for i=1:length(varargin)/3
        ...
        if struct==1
            T=get_apoptosis(varargin{i+(length(varargin)/3)}.tracks);
        else
            T=get_apoptosis_new(varargin{i+(length(varargin)/3)});
        end
        % select the correct N file corresponding the the position chosen
        N=varargin{i};
    ...
    for i=1:length(varargin)/3
        ...
        if struct==1
            T=get_apoptosis(varargin{i+(2*(length(varargin)/3))}.tracks);
        else
            T=get_apoptosis_new(varargin{i+(2*(length(varargin)/3))});
        end
